...
> 从原始脑电数据预处理、频域特征提取、脉冲神经网络（SNN）模型设计，到训练、评估和可交互的「意念打字」Demo，全流程代码 + 中文教程。
...
2. **SNN（脉冲神经网络）**的核心概念：LIF/PLIF 神经元、脉冲发放率编码、替代梯度训练 → [docs/02_snn_explained.md](docs/02_snn_explained.md)
3. **完整工程流程**：数据预处理 → 缓存 → 模型 → 训练 → 评估 → Demo，逐文件讲解 → [docs/03_code_walkthrough.md](docs/03_code_walkthrough.md)
...
> ⚠️ **提示**：本项目的结果是「留出单个被试 S35」的固定拆分，不是严格的 35 折 LOSO（留一被试交叉验证），与论文数值不能直接对比。LOSO 的评估代码在 `evaluate.py` 里已经写好。详见 [docs/03_code_walkthrough.md](docs/03_code_walkthrough.md) 的「局限与改进方向」。
