"""V7训练脚本：5子带Filter-Bank + V5最优训练策略
===============================================
基于V5（数据增强 + 固定拆分），核心升级5子带FB。

训练策略（与V5一致）：
- 固定拆分：S1-S34训练，S35测试
- 数据增强：弱类别（A, F, 4）3倍增强
- Adam + 余弦退火，学习率1e-3
- dropout=0.3, T_snn=12, patience=80
- 早停 + 最佳模型保存
"""
import os
import sys
...
def run_fixed_split(data_dir, cache_dir, model_type='v7_snn', epochs=300, lr=0.001,
    ...
    print(f"V7 Config: 5-band FB, dropout={drop_rate}, Adam, T_snn={T_snn}, patience={patience}")
    print(f"Data Augmentation: factor={augment_factor}, classes={augment_classes}")
    os.makedirs(save_dir, exist_ok=True)
    
    # Load S1-S34 training data
    train_data_list = []
    train_labels_list = []
    for s in range(1, 35):
        try:
    ...
    # Create model
    if model_type == 'v7_ann':
        model = FBSpikeSSVEPformerV7ANN(
    ...
    print(f"\n{'='*60}")
    print(f"V7 FINAL RESULTS ({model_type}, 5-band FB, augmentation x{augment_factor})")
    print(f"{'='*60}")
